...
PROJECT_SCHEMA = "alc.translate.project.v2"
...
class TranslationProject:
    ...
    def open(cls, value: str | Path) -> "TranslationProject":
        root = Path(value)
        marker = root / ".alc" / "translate" / "project.json"
        if root.exists():
            if not root.is_dir():
            ...
            if (
                ...
                and not _has_alc_project_state(root)
            ):
                raise TranslationProjectError(
    ...
    def load(cls, value: str | Path) -> "TranslationProject":
        ...
        if not root.is_dir() or not marker.is_file():
            raise TranslationProjectError(
                "project_not_found", "alc-translate project does not exist"
            )
        project = cls(root.resolve())
        project._read()
        return project
    ...
    def runtime_root(self) -> Path:
        return self.root / ".alc" / "translate"
...
def _has_alc_project_state(root: Path) -> bool:
    """Allow package-specific state under an already initialized ALC project."""

    alc_root = root / ".alc"
    if not alc_root.is_dir():
        return False
    return any(
        (alc_root / name).exists()
        for name in (
            "companion",
            "ocr-proofread",
            "translate",
