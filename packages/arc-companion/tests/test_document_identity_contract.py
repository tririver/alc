...
def test_request_identity_encodes_authors_and_complete_reader_labels(
    ...
    assert encoded["schema_version"] == "arc.companion.build_request.v6"
...
def test_request_decoder_accepts_v4_source_page_label(
    ...
    current.pop("structure_ref")
    current.pop("companion_section_ids")
...
def test_recipe_identity_reserves_author_prompt_and_decodes_v5(
    ...
    assert encoded["schema_version"] == "arc.companion.generation_recipe.v15"
    assert "chapter_plan_prompt" not in encoded
...
def test_generation_recipe_v10_decodes_to_compact_source_contracts() -> None:
    legacy = encode_generation_recipe(CompanionGenerationRecipe())
    legacy["schema_version"] = "arc.companion.generation_recipe.v10"
    legacy["chapter_plan_prompt"] = "arc.companion.chapter-plan-prompt.v9"
    legacy["chapter_guide_prompt"] = (
        "arc.companion.chapter-learning-prompt.v8"
    )
    legacy["chapter_guide_review_prompt"] = (
        "arc.companion.chapter-learning-review-prompt.v8"
    )

    assert decode_generation_recipe(legacy) == CompanionGenerationRecipe()


def test_generation_recipe_v11_decodes_to_advisory_source_access() -> None:
    legacy = encode_generation_recipe(CompanionGenerationRecipe())
    legacy["schema_version"] = "arc.companion.generation_recipe.v11"
    legacy["author_identity_prompt"] = (
        "arc.companion.author-identity-prompt.v2"
    )
    legacy["chapter_plan_prompt"] = "arc.companion.chapter-plan-prompt.v10"
    legacy["chapter_guide_prompt"] = (
        "arc.companion.chapter-learning-prompt.v9"
    )
    legacy["chapter_guide_review_prompt"] = (
...
def test_generation_recipe_v13_decodes_to_current_review_contract() -> None:
    legacy = encode_generation_recipe(CompanionGenerationRecipe())
    legacy["schema_version"] = "arc.companion.generation_recipe.v13"
    legacy["chapter_guide_prompt"] = (
        "arc.companion.chapter-learning-prompt.v10"
    )
    legacy["chapter_guide_review_prompt"] = (
...
def test_generation_recipe_v14_decodes_to_current_science_audit() -> None:
    legacy = encode_generation_recipe(CompanionGenerationRecipe())
    legacy["schema_version"] = "arc.companion.generation_recipe.v14"
    legacy["chapter_guide_prompt"] = (
        "arc.companion.chapter-learning-prompt.v11"
    )
    legacy["chapter_guide_review_prompt"] = (
