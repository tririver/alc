...
from ac_jobs import atomic_write_bytes, atomic_write_json, file_lease


PROJECT_SCHEMA = "alc.companion.project.v3"
DIAGNOSTICS_SCHEMA = "alc.companion.source_diagnostics.v1"
...
class CompanionProjectPaths:
    ...
    def load(cls, value: str | Path) -> "CompanionProjectPaths":
        ...
        root = Path(value)
        marker = root / ".alc" / "companion" / "project.json"
    ...
    def runtime_root(self) -> Path:
        return self.root / ".alc" / "companion"
    ...
    def _initialization_conflict(self) -> Path | None:
        ...
        alc_root = self.root / ".alc"
        if _path_exists(alc_root) and not alc_root.is_dir():
            return alc_root
        for path in (
            self.runtime_root,
            self.delivery_html,
