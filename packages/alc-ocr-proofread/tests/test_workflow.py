...
from ac_jobs import ImmutableArtifactStore, ResumeReason, RunStatus
from ac_llm import LLMCompleted, LLMPaused
from ac_document import RenderedPDFPage

from alc_ocr_proofread import ProofreadProject, ProofreadService, load_mineru_source
from alc_ocr_proofread.workflow import (
    PAGE_OUTPUT_SCHEMA,
...
def _bundle(tmp_path: Path, monkeypatch, *, pages: int = 1):
    ...
    monkeypatch.setattr("alc_ocr_proofread.source.subprocess.run", fake_run)
...
def test_boundary_repair_reuses_verified_delivery(
    ...
    assert review["schema_version"] == "alc.ocr_proofread.boundary_review_request.v3"
    snapshot = service.resume(
        snapshot.run_id,
...
def test_durable_review_audit_and_delivery(tmp_path: Path, monkeypatch) -> None:
    ...
    assert snapshot.status is RunStatus.PAUSED
    review = _artifact_json(service, snapshot)
    assert review["schema_version"] == "alc.ocr_proofread.review_request.v1"
    review_input = {
    ...
    assert snapshot.status is RunStatus.PAUSED
    audit = _artifact_json(service, snapshot)
    assert audit["schema_version"] == "alc.ocr_proofread.audit_request.v1"
...
def test_no_review_items_pauses_directly_for_audit(tmp_path: Path, monkeypatch) -> None:
    ...
    assert snapshot.status is RunStatus.PAUSED
    assert _artifact_json(service, snapshot)["schema_version"] == "alc.ocr_proofread.audit_request.v1"
