...
def _write_minimal_arc_repo(work: Path) -> None:
    ...
    (work / "packages/arc-ocr-proofread/src/arc_ocr_proofread/__init__.py").write_text(
        '__version__ = "0.1.0"\n',
        encoding="utf-8",
    )
    (work / "packages/arc-translate/src/arc_translate").mkdir(parents=True)
    (work / "packages/arc-translate/src/arc_translate/__init__.py").write_text(
        '__version__ = "0.1.0"\n',
        encoding="utf-8",
    )
...
def _apply_release_bump(work: Path, version: str = "0.2.0") -> None:
    ...
    for host in ("codex", "claude"):
        _replace(
            work / f"plugins/arc/.{host}-plugin/plugin.json",
            '"version": "0.1.0"',
            f'"version": "{version}"',
        )
    _replace(
...
def test_release_script_does_not_reference_retired_arc_mcp() -> None:
    source = SCRIPT.read_text(encoding="utf-8")
    assert "arc-mcp" not in source
    assert "arc_mcp" not in source
...
def test_release_script_bumps_versions_creates_one_tag_and_pushes_stable(tmp_path: Path) -> None:
    ...
    assert '"arc-paper>=0.2,<0.3"' in (
        work / "packages/arc-ocr-proofread/pyproject.toml"
    ).read_text(encoding="utf-8")
    ...
    assert not (work / "packages/arc-mcp").exists()
    assert not (work / "plugins/arc-mcp").exists()
...
def test_release_script_validates_claude_manifest_without_claude_tag(tmp_path: Path) -> None:
    ...
    assert "plugin validate plugins/arc-mcp" not in call_text
